...
public class WareHouse2DAO {
	...
	public int insertWare(WareHouse1DTO dto) {
		...
		try {
			conn=DB.oraConn();
			String sql="insert into warehouse2 values(?,?,?,?,?,?)";
			pstmt=conn.prepareStatement(sql);
	...
	public int deleteWare(String num) {
		...
		try {
			conn=DB.oraConn();
			String sql="delete warehouse2 where num=?";
			pstmt=conn.prepareStatement(sql);
	...
	public int UpdateWare(WareHouse1DTO dto) {
		...
		try {
			conn=DB.oraConn();
			String sql="update warehouse2 set name=?, company=?, amount=?, price=?, volume=? "
					+ " where num=?";
			pstmt=conn.prepareStatement(sql);
